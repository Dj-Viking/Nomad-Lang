...
//   async getUserCards(@Ctx() { req }: MyContext): Promise<GetUserCardsResponse> {
...
//   async clearUserCards(@Ctx() { req }: MyContext): Promise<ClearCardResponse | ErrorResponse> {
...
//     if (req.user?.email !== foundUserByEmail.email)
//       return new ErrorResponse("forbidden", "403 Forbidden");
...
//     const { backSideLanguage, frontSideText, backSideText, frontSideLanguage, frontSidePicture } =
//       options;
...
//     }
//     const foundUserByEmail: User | undefined = await User.findOne({
